        ...
        <div class="areashowshoes">
                            ...
                            <ul>
                                <?php 
                                 $conn = mysqli_connect("localhost", "root", "", "db.websellsshoes");
                                    $sqlCom2 = "SELECT DISTINCT thuonghieu FROM khohang";
                                    $kqkt = mysqli_query($conn, $sqlCom2);
                                    while ($row = mysqli_fetch_array($kqkt)) {

                                            echo '<a href="thuonghieu.php?thuonghieu='.$row['thuonghieu'].'"> <li> '.$row['thuonghieu'].' </li></a> ';
                    ...
                    <div id="show">
                        <div id="areashow">
                                ...
                                $thuonghieu  = (isset($_GET['thuonghieu'])) ? $_GET['thuonghieu'] : null;
                                
                                    if($thuonghieu != null)
                                    {
                                
                                        unset($_SESSION['totalPages']);
                                        $sqlCom2 = "SELECT * FROM khohang where thuonghieu like '$thuonghieu' ";
                                        $kqkt = mysqli_query($conn, $sqlCom2);
                                        $row_count = mysqli_num_rows($kqkt); 
                                        $_SESSION['rowcount'] = $row_count;      
                                        // Tính toán số lượng trang
                                    ...
                                    if(isset($_SESSION['totalPages']))
                                    {
                                        if($_SESSION['totalPages']!=1)
                                        {
                                                ...
                                                for ($i = max(1, $current_page - $range); $i <= min($current_page + $range, $total_pages); $i++) {
                                                    echo '<li><a href="?thuonghieu='.$thuonghieu.'&page=' . $i . '"';
                                                    if ($i == $current_page) {
                                                ...
                                                if ($total_pages > 3 && $current_page > 1) {
                                                    echo '<li><a href="?thuonghieu='.$thuonghieu.'&page=' . ($current_page - 1) . '">Trước</a></li>';
                                                }
                                                ...
                                                if ($total_pages > 3 && $current_page < $total_pages) {
                                                    echo '<li><a href="?thuonghieu='.$thuonghieu.'&page=' . ($current_page + 1) . '">Tiếp</a></li>';
                                                }
                                                echo '</ul>';
