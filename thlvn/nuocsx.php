        ...
        <div class="areashowshoes">
                    ...
                    <div id="option">
                            ...
                            <ul>
                                <?php 
                                 $conn = mysqli_connect("localhost", "root", "", "db.websellsshoes");
                                    $sqlCom2 = "SELECT DISTINCT nuocsx FROM khohang";
                                    $kqkt = mysqli_query($conn, $sqlCom2);
                                    while ($row = mysqli_fetch_array($kqkt)) {

                                            echo '<a href="nuocsx.php?nuocsx='.$row['nuocsx'].'"> <li> '.$row['nuocsx'].' </li></a> ';
                    ...
                    <div id="show">
                        <div id="areashow">
                                ...
                                $nuocsx  = (isset($_GET['nuocsx'])) ? $_GET['nuocsx'] : null;

                                if($nuocsx != null)
                                {
                                    $sqlCom2 = "SELECT * FROM khohang where nuocsx like '$nuocsx' ";
                                    $kqkt = mysqli_query($conn, $sqlCom2);
                                    $row_count = mysqli_num_rows($kqkt);
                                    $_SESSION['rowcount'] = $row_count;
                                    $itemsPerPage = 8; 
                                    ...
                                    if(isset($_SESSION['totalPages']))
                                {
                                    if($_SESSION['totalPages']!=1)
                                    {
                                            ...
                                            for ($i = max(1, $current_page - $range); $i <= min($current_page + $range, $total_pages); $i++) {
                                                echo '<li><a href="?nuocsx='.$nuocsx.'&page=' . $i . '"';
                                                if ($i == $current_page) {
                                            ...
                                            if ($total_pages > 3 && $current_page > 1) {
                                                echo '<li><a href="?nuocsx='.$nuocsx.'&page=' . ($current_page - 1) . '">Trước</a></li>';
                                            }
                                            ...
                                            if ($total_pages > 3 && $current_page < $total_pages) {
                                                echo '<li><a href="?nuocsx='.$nuocsx.'&page=' . ($current_page + 1) . '">Tiếp</a></li>';
                                            }
                                            echo '</ul>';
